...
<form action="/Preguntas/{{$pregunta->id}}" method="post" enctype="multipart/form-data" style="display: inline;" >
  ...
  @if (is_null($pregunta->p_autorizada))
    <div class="alert alert-info" role="alert">
      {{$pregunta->quien->nombre}} pregunto <small class="text-muted initialism">{{$pregunta->hora_p}}</small> : 
      <textarea name="pregunta" class="form-control" rows="3">{{$pregunta->pregunta}}</textarea>
    </div>
  @else
    <div class="alert alert-info" role="alert">
...
<form action="/Preguntas/{{$pregunta->id}}" method="post" style="display: inline;"  onsubmit="return confirm('Desea eliminar');">
  @csrf
  @method('DELETE')
